...
LLM_MODEL = os.getenv('LLM_MODEL', 'gpt-3.5-turbo-1106')  # Provide a default value if not set
...
async def main():
    ...
    #get username from argument if not supplied ask for it
    if len(sys.argv) > 1:
    ...
    # Convert generators to lists before concatenating
    all_videos = list(videos) + list(streams)
    count = 0
    ...
    for vid in all_videos:
        ...
        if safe_video_title in os.listdir(f"transcripts/{channel_username}"):
            ...
            continue
        print("Generating transcript for video:", video_title)
        transcript = await generate_transcript(vid_id)
        if transcript is None:
            continue  # Skip further processing for this 
        
        #If secong argument exiist and is "--raw" then skip punctuation
        if len(sys.argv) > 2 and sys.argv[2] == "--raw":
            #just use the transcript as is and write it to file
